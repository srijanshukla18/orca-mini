...
# build.sh — Archive, export, notarize, and package Orca Mini into a DMG.
#
...
#
# Output: build/Orca-Mini-<version>-arm64.dmg (signed, notarized, stapled)
...
step "Building Orca Mini v${VERSION}"
...
APP_PATH="$EXPORT_PATH/Orca Mini.app"
[[ -d "$APP_PATH" ]] || die "Export failed — ${APP_PATH} not found."

echo "Copying exported app bundle..."
ditto "$APP_PATH" "build/Orca Mini.app"

echo "Verifying exported app signature..."
codesign --verify --deep --strict --verbose=4 "build/Orca Mini.app" >/dev/null || die "App signature verification failed after export."
...
create-dmg \
    ...
    --skip-jenkins \
    "build/${DMG_NAME}" \
    "build/Orca Mini.app" 2>/dev/null || true
...
xcrun stapler staple "build/Orca Mini.app"
